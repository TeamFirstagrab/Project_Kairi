...
public class TestHooking : MonoBehaviour
{
	...
	public float nodeDist = 0.5f;   // 점 사이의 거리
	private float lineDist;			// 줄 발사 거리
	private bool isAttach = false;	// 플레이어가 갈고리를 사용했는지
	...
	private void Start()
	{
		...
		lineDist = GameManager.Instance.playerStatsRuntime.hookDistance;
	...
	private void Update()
	{
        ...
        if ((Vector2)transform.position != destiny)
		{
			...
			while (Vector2.Distance(player.transform.position, lastNode.transform.position) > nodeDist)	CreateNode();
		}
	...
	void RenderLine()
	{
		line.positionCount = segmentCnt;     // 라인렌더러 갯수를 점 갯수만큼 설정
